...
filtered_flights <- flights %>% 
  filter(arr_delay >= 2 & dest == "IAH" & month == 7 | month == 8 | month == 9)
...
```{r}
flights %>% 
  select(dep_time:arr_delay) %>% head()
```
...
flights %>% 
  mutate(dep_time_minute = (dep_time - (dep_time %% 100))*60 + dep_time %% 100) %>% 
  mutate(sched_dep_time_minute = (sched_dep_time - (sched_dep_time %% 100))*60 + sched_dep_time %% 100) %>%
  select(dep_time_minute, sched_dep_time_minute) %>% 
  head()
```
